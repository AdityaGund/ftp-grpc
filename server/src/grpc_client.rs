...
pub async fn transfer_data(
    ...
    // NEW: Destination label for clearer log output
    let dest_label = destination.unwrap_or("unknown");
    ...
    if let Some((file_path, _)) = file_details {
        ...
        loop {
            ...
            // Keep reading until we fill CHUNK_SIZE or hit EOF
            ...
            println!("[ADMIN] [{}] created chunk {}", dest_label, i);
            i += 1;
        }

        total_chunks = file_chunks.len() as i32;
        println!("[DEBUG] [{}] Total chunks calculated: {}", dest_label, total_chunks);
    }
    ...
    if !file_chunks.is_empty() {
        for (idx, chunk) in file_chunks.into_iter().enumerate() {
            ...
            println!("[ADMIN GRPC] [{}] Sending chunk {}/{}", dest_label, idx + 1, total_chunks);
